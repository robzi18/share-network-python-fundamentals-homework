...
points = [1, 3, 3, 2, 1, 4, 2, 4, 1, 8, 5, 1,
          3, 1, 1, 3, 10, 1, 1, 1, 1, 4, 4, 8, 4, 10]
boo = dict(list(zip(letters, points)))


def score_word(user_word):
    point_total = 0
    for letter in user_word:
        point_total += boo[letter]
...
while other_player:
    ...
    while continue_game:
        ...
        total_score = score_word(user_word)
        if player_name not in player_to_words:
            player_to_words[player_name] = []
        player_to_words[player_name].append(user_word)
        user_option = input("Do you want to keep playing. Yes or No \n")
    ...
    is_there_player = input("Is there any other player? Yes or No\n")
    if is_there_player in ("yes", "Y", "y", "Yes"):
        other_player = True
    else:
        other_player = False
...
for player in player_to_words:
    ...
    # list_words.append(player_to_words[list_player])
    for word in player_to_words[player]:
        # print(f"Hi this are the words {word} >> the point is {pt} \n")
        player_points += score_word(word)

    print(f"Total point for {player} is {player_points}.")
